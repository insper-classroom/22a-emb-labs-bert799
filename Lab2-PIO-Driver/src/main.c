...
const uint32_t ul_mask){
		...
		if (ul_type == PIO_OUTPUT_0) {
			...
			in_or_out = p_pio->PIO_ODSR; 
			} else {
			in_or_out = p_pio->PIO_PDSR;  
		}

		if ((in_or_out & ul_mask) == 0) {  
			return 0;
			} else {
			return 1;
		}
...
void blinkLED (Pio * led, const uint32_t led_mask){
	for (int i = 0; i < 2; i++){
		_pio_clear(led, led_mask);      // Coloca 1 no pino LED da placa
		_delay_ms(100);                        // Delay por software de 100 ms
		_pio_set(led, led_mask);    // Coloca 0 no pino do LED
		_delay_ms(100);                        // Delay por software de 100 ms
	}
}
